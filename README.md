...
> 📌 **第一次来？** 先看 [**问题一：机构订阅文献库拿不到全文**](#问题一机构订阅文献库拿不到全文本项目存在的根本原因)
> （这是本项目存在的理由），再看 [`examples/演示输出.md`](examples/演示输出.md)（无需安装，直接看结果）。
>
> 🔒 **关于隐私**：本仓库内**不含**真实姓名、学校、院系、联系方式，
> 也**不含**任何未发表论文的题目与正文——所有示例均已脱敏（虚构作者与文献）。
> 若你要基于本项目发布自己的版本，请先跑 `python scripts/check_pii.py .`，
> 详见 [`references/16-privacy-protection.md`](references/16-privacy-protection.md)。
...
**本项目的解法：不硬碰那条走不通的自动化路径，而在正确的边界上切一刀。**
